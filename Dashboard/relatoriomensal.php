...
$query2 = "select sum(cast(REPLACE(valor, ',', '.') as decimal(8,2))) as faturou from faturamento
 where idColaborador ='$idColaborador'  and MONTH(`dataVenda`) = MONTH(CURRENT_DATE())";
...
$query = $conn->prepare("SELECT faturamento.*, temp_produtos.* from faturamento
							...
							where faturamento.idColaborador= '$idColaborador' and MONTH(`dataVenda`) = MONTH(CURRENT_DATE())");
$query->execute();
